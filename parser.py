...
class InshortsParser:
    ...
    def fetch_summaries(self):
        """Fetch article summaries with load more simulation"""
        summaries = []
        news_offset = None
        attempt = 0
        
        while len(summaries) < self.max_news and attempt < 3:
            try:
                ...
                # Parse summaries (changed from headlines)
                ...
                print(f"Batch {attempt + 1}: Added {len(new_summaries)} summaries (Total: {len(summaries)})")
        ...
        return summaries[:self.max_news]

if __name__ == "__main__":
    print("Fetching latest technology summaries...")
    parser = InshortsParser(max_news=20)
    summaries = parser.fetch_summaries()
    
    print(f"\nFinal Results ({len(summaries)} summaries):")
    for idx, summary in enumerate(summaries, 1):
        print(f"\nSummary {idx}:")
        print(summary)
    
    if len(summaries) < 20:
        print(f"\nNote: Only {len(summaries)} summaries were available")
